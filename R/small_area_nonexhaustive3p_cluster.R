...
small_area_nonexhaustive3p_cluster<- function(formula.s0, formula.s1, data, phase_id, cluster, small_area, boundary_weights, thresh.n2g, psmall){
  ...
  # data_clust_s1<- merge(cluster_means.s1, clust_info, by="cluster") # attach clusterinfo incl. indicator variable
  # create and add phase_id on cluster-level:
  # data_clust.s1_groundindicator <- merge(data_clust.s1, aggregate(data[,c(cluster, phase_id[["phase.col"]])], list(data[,cluster]), unique)[-1], by=cluster, all.x=TRUE)
  ...
  # create and add phase_id on cluster-level:
  # data_clust_1.s1_groundindicator <- merge(data_clust_1.s1, aggregate(data[,c(cluster, phase_id[["phase.col"]])], list(data[,cluster]), unique)[-1], by=cluster, all.x=TRUE)
  ...
  n0_clusters <- nrow(aggregate(!(data[[phase.col]] %in% c(s1.ind, s2.ind)), list(cluster = data[[cluster]]), length))
  ...
  # compute data_ext_1.s0G i.e dataset of reduced model for s0G-sample:
  design_matrix_1.s0G_plot_level <- design_matrix.s1_return(formula=formula.s0, data=data[data[,sa.col] == 1 ,])
  data_ext_1.s0G <- data.frame(design_matrix_1.s0G_plot_level)
  data_ext_1.s0G[,all.vars(formula.s0)[1]] <- data[data[,sa.col] == 1, all.vars(formula.s0)[1]]
  data_ext_1.s0G[,cluster] <- data[data[,sa.col] == 1, cluster]
  data_ext_1.s0G[,phase.col] <- data[data[,sa.col] == 1, phase.col]
  if(!is.na(boundary_weights)){
    data_ext_1.s0G[,boundary_weights]<- data[data[,sa.col] == 1, boundary_weights]
  }
  ...
  # Mx_s0G (should theoretically be identical to Mx_s1G):
  cluster_weights.s0G<- aggregate(data_ext_1.s0G[,all.vars(formula.s0)[1]], list(cluster = data_ext_1.s0G[,cluster]), length) # Mx_s0G
  colnames(cluster_weights.s0G)[2]<- "Mx_s0G"
  ...
  ## ... of reduced model for s0G:
  data_clust_1.s0G <- aggregate(data_ext_1.s0G[,-which(names(data_ext_1.s0G) == cluster)], list(cluster = data_ext_1.s0G[,cluster]), mean)
  # for weighted cluster-means:
  if(!is.na(boundary_weights)){data_clust_1.s0G<- boundaryweight_fct_3p_clust(formula.s0, data_ext_1.s0G, boundary_weights, cluster)}
  data_clust_1.s0G<- merge(data_clust_1.s0G, cluster_weights.s0G, by=cluster)
  ...
  ## ... of reduced model for s0G:
  design_matrix_1.s0G<- as.matrix(data_clust_1.s0G[ , -c(which(names(data_clust_1.s0G) %in% c(cluster,all.vars(formula.s0)[1], "Mx_s0G", phase.col)))])# Zc_1(x)
  ...
  M_x.s0G<- data_clust_1.s0G[["Mx_s0G"]]
  M_x.s2G<- data_clust.s2G[["Mx_s2G"]]


  # get sample size:
  n2G_plots<-  nrow(data_ext.s2G)
  n1G_plots<-  nrow(data_ext.s1G)
  n0G_plots<-  nrow(data_ext_1.s0G)
  n0G_clusters <- nrow(design_matrix_1.s0G)
  n1G_clusters <- nrow(design_matrix.s1G)
  n2G_clusters <- nrow(data_clust_1.s2G)
  ...
  # ------------------------------------ #
  # 3) "normal" case (some n2g available):
        ...
        if(!psmall){
          w4<- warning(
            paste("At least one terrestrial cluster not entirely included within the small area ",small_area[["areas"]],".\n",
                  "Zero mean residual assumption for small area may be violated.","\n",
  ...
  Z_1_bar_s0G<- apply(design_matrix_1.s0G, 2, weighted.mean, w=M_x.s0G) # that's Z_bar(1)_1 in Daniels Report...

  # calculate covariance matrix of Z_bar_1_s0: (page 10, [27])
  design_matrix_1.s0G_centered <- t(apply(design_matrix_1.s0G, 1, function(Zx,...){Zx - Z_1_bar_s0G}))
  design_matrix_1.s0G_centered_weighted <- apply(design_matrix_1.s0G_centered, 2, function(col){as.vector(M_x.s0G / mean(M_x.s0G))*col})
  cov_Z_bar_1_s0G <- (t(design_matrix_1.s0G_centered_weighted) %*% design_matrix_1.s0G_centered_weighted)/(n0G_clusters*(n0G_clusters-1))# eq [64] p. 26 Mandallaz 2p/3p technical report
  ...
  # db-estimates:
  estimate<- ((Z_1_bar_s0G - Z_1_bar_s1G) %*% alpha) + (Z_bar_s1G %*% beta)

  g_variance<- t(alpha) %*% cov_Z_bar_1_s0G %*% alpha +
               (n2_clusters / n1_clusters) * (Z_1_bar_s0G %*% cov_alpha_s2 %*% Z_1_bar_s0G) +
